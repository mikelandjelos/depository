...
│   ├── daily_art.yaml              # Generated daily Met artwork metadata — do not hand-edit
│   ├── post_cards.yaml             # Generated card shape/color/frame manifest — do not hand-edit
│   ├── references.yaml             # Citation records keyed for Hugo shortcodes
│   └── site_refresh.yaml           # Generated daily deployment trigger — do not hand-edit
├── docs/
...
│   └── refresh_daily_art.py       # Selects and caches daily public-domain Met artwork
...
│       ├── refresh-daily-art.yml  # Nightly artwork refresh and deployment trigger
...
- **Daily art is build-time content**: `refresh_daily_art.py` selects a
  deterministic daily member of the Met's public domain highlight-painting
  collection, downloads it into `static/images/daily_art.jpg`, and records
...
- **Citations are native Hugo shortcodes**: Markdown, Quarto, and notebook
  posts cite `{{< cite key >}}` and end with `{{< references >}}`. The shortcodes
